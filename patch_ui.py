...
file_path = "frontend/src/components/parent/chat/NutriGuideChat.jsx"

with open(file_path, "r", encoding="utf-8") as f:
    content = f.read()

# --- Commit 2 & 4: Render Structure & Cards ---
new_render_content = """    // Enhanced Markdown Renderer
...
content = re.sub(r'    // Enhanced Markdown Renderer.*?    return \(\n        <div className="flex flex-col', new_render_content + '\n\n    return (\n        <div className="flex flex-col', content, flags=re.DOTALL)

new_details_section = """const DetailsSection = ({ content, formatText, renderSections, detailSections }) => {
...
content = re.sub(r'const DetailsSection = \(\{ content, formatText \}\) => \{.*?\n\};', new_details_section, content, flags=re.DOTALL)

with open(file_path, "w", encoding="utf-8") as f:
    f.write(content)

subprocess.run(["git", "add", file_path])
subprocess.run(["git", "commit", "-m", "refactor: improve chatbot response rendering structure"])
...
content = re.sub(r'                        <div>\s+<h2 className="text-lg font-bold text-slate-800 dark:text-white leading-tight">NutriGuide AI</h2>\s+<p className="text-xs text-slate-500 font-medium">\s+\{selectedChild \? `Advising for: \$\{selectedChild.name\}` : "General Advice Mode"\}\s+</p>\s+</div>', header_replacement, content)

with open(file_path, "w", encoding="utf-8") as f:
    f.write(content)

subprocess.run(["git", "add", file_path])
subprocess.run(["git", "commit", "-m", "feat: redesign chat interface for professional healthcare UX and implement verified source indicators"])

# --- Commit 6: Voice Interaction ---
with open(file_path, "r", encoding="utf-8") as f:
    content = f.read()

voice_state = """    const [isTyping, setIsTyping] = useState(false);
...
content = content.replace("    const messagesEndRef = useRef(null);", voice_state)

voice_effect = """    useEffect(scrollToBottom, [messages, isTyping]);
...
content = content.replace("    useEffect(scrollToBottom, [messages, isTyping]);", voice_effect)

voice_button = """                <form onSubmit={(e) => { e.preventDefault(); handleSend(); }} className="relative flex gap-3">
                    <button
                        type="button"
...
content = content.replace('                <form onSubmit={(e) => { e.preventDefault(); handleSend(); }} className="relative flex gap-3">\n                    <div className="flex-1 relative">', voice_button)

with open(file_path, "w", encoding="utf-8") as f:
    f.write(content)

subprocess.run(["git", "add", file_path])
subprocess.run(["git", "commit", "-m", "feat: integrate speech-to-text voice interaction for chat input"])
