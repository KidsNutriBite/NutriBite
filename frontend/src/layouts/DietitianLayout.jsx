...
const DietitianLayout = ({ children }) => {
    ...
    const getStatusColor = (status) => {
        switch (status) {
            case 'Available': return 'bg-green-500 ring-green-200';
            case 'Busy': return 'bg-amber-500 ring-amber-200';
            case 'Offline': return 'bg-rose-500 ring-rose-200';
        ...
        <div className="bg-background-light dark:bg-background-dark font-display min-h-screen text-slate-800 dark:text-slate-200">
            ...
            <header className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md px-6 md:px-20 py-4 sticky top-0 z-50">
                <div className="flex items-center gap-3">
                    ...
                    <Link href="/dietitian/dashboard" className="text-slate-900 dark:text-white text-2xl font-extrabold tracking-tight hidden sm:block">NutriKid</Link>
                    <Link href="/dietitian/dashboard" className="text-slate-900 dark:text-white text-xl font-extrabold tracking-tight sm:hidden">NutriKid</Link>
                </div>
                ...
                <nav className="hidden md:flex items-center gap-2">
                    <Link
                        ...
                        className={isActive('/dietitian/dashboard')
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="relative flex items-center gap-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-3 py-1.5 rounded-xl shadow-sm">
                        <span className={`h-2.5 w-2.5 rounded-full ring-4 ${getStatusColor(availability)}`}></span>
                        <select
                            value={availability}
                    ...
                    <div className="relative" ref={profileRef}>
                        ...
                        >
                            <div className="text-right hidden sm:block">
                                <p className="text-sm font-bold leading-none">{user?.name || 'Dietitian'}</p>
                            ...
                            <div className="absolute right-0 mt-4 w-60 bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-100 dark:border-slate-700 overflow-hidden z-50 ring-1 ring-black/5 animate-in fade-in zoom-in duration-200 origin-top-right">
                                ...
                                <div className="p-2 space-y-1">
                                    <Link
                                        ...
                                        className="flex items-center gap-3 px-4 py-3 text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50 rounded-xl transition-colors group"
                                    >
                                        <div className="p-1.5 rounded-lg bg-slate-50 dark:bg-slate-800 text-slate-500 group-hover:bg-primary group-hover:text-white transition-colors">
                                    ...
                                    <button
                                        ...
                                        className="flex items-center gap-3 w-full px-4 py-3 text-sm font-medium text-red-600 hover:bg-red-50 dark:hover:bg-red-900/10 rounded-xl transition-all group"
                                    >
                ...
                <div className="md:hidden bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 p-4 sticky top-[73px] z-40 shadow-lg">
                    <nav className="flex flex-col gap-2">
                        <Link
                            ...
                            className={isActive('/dietitian/dashboard') ? "bg-primary/10 text-primary font-bold px-4 py-3 rounded-xl" : "text-slate-600 dark:text-slate-400 font-medium px-4 py-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"}
                        >
                            <div className="flex items-center gap-3">
