...
const DoctorProfile = () => {
    ...
    const fetchProfile = async () => {
        try {
            ...
            setFormData({
                ...
                doctorProfile: {
                    specialization: data.data.doctorProfile?.specialization || '',
                    hospitalName: data.data.doctorProfile?.hospitalName || '',
                    experienceYears: data.data.doctorProfile?.experienceYears || 0,
                    registrationId: data.data.doctorProfile?.registrationId || ''
                }
            });
            setLoading(false);
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            submitData.append('doctorProfile', JSON.stringify(formData.doctorProfile));

            if (selectedFile) {
                submitData.append('profileImage', selectedFile);
            }

            const { data } = await api.patch('/doctor/update', submitData, {
    ...
    return (
        <div className="max-w-4xl mx-auto">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Doctor Profile</h1>
                {!isEditing && (
                    <button
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-hidden">
                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-slate-700 dark:to-slate-800 p-8 flex flex-col md:flex-row items-center gap-6 border-b border-gray-100 dark:border-slate-700">
                    ...
                    <div className="text-center md:text-left">
                        ...
                        <span className="inline-block mt-2 px-3 py-1 bg-primary text-white text-xs font-semibold rounded-full uppercase tracking-wider">
                            {profile?.role}
                        </span>
                    </div>
                </div>
                    ...
                    {isEditing ? (
                        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="md:col-span-2">
                                <h3 className="text-lg font-medium text-gray-800 dark:text-gray-200 mb-3 border-b pb-2">Professional Details</h3>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    value={formData.doctorProfile.hospitalName}
                                    onChange={handleChange}
                                    className="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white focus:ring-2 focus:ring-blue-500"
                                />
                            </div>
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Medical Registration ID</label>
                                <input
                                    type="text"
                                    name="doctorProfile.registrationId"
                                    value={formData.doctorProfile.registrationId}
                                    onChange={handleChange}
                                    className="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white focus:ring-2 focus:ring-blue-500"
                                />
                            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                            <div className="flex flex-col">
                                ...
                                <span className="font-medium text-gray-800 dark:text-gray-200">{profile?.doctorProfile?.specialization || 'Not set'}</span>
                            </div>
                            <div className="flex flex-col">
                                <span className="text-sm text-gray-500 dark:text-gray-400">Hospital</span>
                                <span className="font-medium text-gray-800 dark:text-gray-200">{profile?.doctorProfile?.hospitalName || 'Not set'}</span>
                            </div>
                            <div className="flex flex-col">
                                <span className="text-sm text-gray-500 dark:text-gray-400">Experience</span>
                                <span className="font-medium text-gray-800 dark:text-gray-200">{profile?.doctorProfile?.experienceYears || 0} Years</span>
                            </div>
                            <div className="flex flex-col">
                                <span className="text-sm text-gray-500 dark:text-gray-400">Registration ID</span>
                                <span className="font-medium text-gray-800 dark:text-gray-200">{profile?.doctorProfile?.registrationId || 'Not set'}</span>
                            </div>
                            <div className="flex flex-col">
